...
import { ResendVerificationButton } from "@/components/billing/resend-verification-button";

export function AccessBanner({ status }: { status: AccessStatus }) {
  if (!status.message) return null;

  const isEmailState = status.state === "email_pending" || status.state === "email_unverified";
  const isSevere = status.state === "past_due" || status.state === "locked" || status.state === "email_unverified";
  ...
  return (
    ...
    >
      ...
      {isEmailState ? (
        <ResendVerificationButton isSevere={isSevere} />
      ) : (
        <Link
