  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 print:border-gray-300 print:p-2">
      <div className="flex items-center justify-between mb-2">
        <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-700 border border-slate-200 print:bg-gray-100 print:text-black">
          {badge}
        </span>
...
export function RateConfirmationView({
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 rounded-2xl border border-slate-200 bg-white p-8 text-slate-900 shadow-sm print:max-w-none print:border-none print:shadow-none print:p-0 print:text-black print:bg-white">
      ...
      <div className="grid gap-4 sm:grid-cols-2 border-b border-slate-200 pb-5 print:border-gray-300">
        <div className="rounded-xl bg-slate-50/70 p-4 border border-slate-200 print:bg-white print:border-gray-200">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-600 print:text-black">
            Carrier On Record
          </span>
          {carrier ? (
            ...
            <p className="mt-2 text-xs text-slate-500 italic">No carrier assigned yet.</p>
          )}
        </div>
        ...
        <div className="rounded-xl bg-slate-50/70 p-4 border border-slate-200 print:bg-white print:border-gray-200">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-600 print:text-black">
            <p>Equipment: <span className="font-semibold text-slate-900 print:text-black">{load.equipmentType || "53' Van"}</span></p>
            <p>Commodity: <span className="font-semibold text-slate-900 print:text-black">{load.commodity || "Freight (FAK)"}</span></p>
            <p>Weight: <span className="font-semibold text-slate-900 print:text-black">{load.weightLbs ? `${load.weightLbs.toLocaleString()} lbs` : "42,000 lbs"}</span></p>
            <p>Temp Setting: <span className="font-semibold text-slate-900 print:text-black">{load.temperatureSetting || "Dry / Ambient"}</span></p>
          </div>
        </div>
      </div>
      ...
      <section className="space-y-3">
        <h2 className="text-xs font-bold uppercase tracking-wider text-slate-900 print:text-black">
          Schedule & Route Itinerary
        </h2>
        <div className="grid gap-4 sm:grid-cols-2">
          <StopBlock stop={load.origin} badge="Stop 1 · Shipper Origin" />
          <StopBlock stop={load.destination} badge="Stop 2 · Consignee Delivery" />
        </div>
      </section>

      {/* Financials */}
      <section className="rounded-xl border border-slate-200 overflow-hidden print:border-gray-300">
        <div className="grid grid-cols-3 bg-slate-50 p-3 text-xs font-bold text-slate-600 border-b border-slate-200 print:bg-gray-100 print:text-black">
        ...
        <div className="divide-y divide-slate-100 text-xs print:divide-gray-200">
          <div className="grid grid-cols-3 p-3">
            <span className="font-medium text-slate-900 print:text-black">Agreed Line Haul Rate</span>
            <span className="text-center text-slate-500 print:text-black">Flat Fee</span>
          ...
          <div className="grid grid-cols-3 bg-blue-50/60 p-3.5 text-sm font-bold text-slate-900 border-t border-blue-100 print:bg-gray-100 print:text-black">
            <span>TOTAL AGREED CARRIER PAY</span>
            <span className="text-center text-xs font-medium text-blue-700 print:text-black">Net 30 / QuickPay</span>
      ...
      {/* Legal terms */}
      <section className="rounded-xl bg-slate-50 p-3.5 text-[10px] text-slate-600 border border-slate-200 print:border-gray-200 print:text-black leading-relaxed">
      ...
      {/* Signatures */}
      <footer className="grid grid-cols-2 gap-8 border-t-2 border-slate-900 pt-6 print:border-black">
        <div>
